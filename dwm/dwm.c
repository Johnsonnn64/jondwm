...
enum { SchemeNorm, SchemeSel, SchemeTag, SchemeTag1, SchemeTag2, 
       SchemeTag3, SchemeTag4, SchemeTag5, SchemeTag6, SchemeTag7, 
       // SchemeTag8, SchemeTag9, 
       SchemeLayout }; /* color schemes */
...
{
	selmon->sel->x = selmon->mx + (selmon->sel->mon->mw - WIDTH(selmon->sel)) / 2;
	selmon->sel->y = selmon->my + (selmon->sel->mon->mh - HEIGHT(selmon->sel)) / 2;
	arrange(selmon);
}
...
togglefullscr(const Arg *arg)
{
  ...
  if(selmon->sel) {
    if ((selmon->showbar + selmon->sel->isfullscreen) == 1) 
      togglebar(NULL);
...
void hideotherwins(const Arg *arg) {
  ...
  for (i = selmon->clients; i; i = i->next) {
    if (i != c && ISVISIBLE(i)) {
      i->tags = selmon->tagset[selmon->seltags];
      hide(i);
